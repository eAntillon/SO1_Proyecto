...
static int escribir_a_proc(struct seq_file *file_proc, void *v)
{
    ...
    long unsigned int freeram = ((uint64_t)info.freeram * info.mem_unit) / (1024 * 1024);
    long unsigned int ramusage = totalram - freeram;
    float percent = totalram * ramusage/100; 
    seq_printf(file_proc, "{\"totalram\": %lu,\"freeram\":%lu, \"ramusage\": %lu}", totalram, freeram, ramusage);
...
static int __init ram_module_init(void)
{
    ...
    printk(KERN_INFO "Modulo de RAM insertado \n");
    return 0;
}
...
static void __exit ram_module_cleanup(void)
{
    ...
    printk(KERN_INFO "Modulo de RAM extraido\n");
}
